...
class GetPostsTest(TestCase):
    def setUp(self):
        ...
        Post.objects.create(content=content1, author='durpal',
                            category='cats', date=date1)
        Post.objects.create(content=content2, author='bartek',
                            category='cats', date=date2)
        Post.objects.create(content=content3, author='piotr',
                            category='snakes', date=date3)
    ...
    def test_get_post_can_select_proper_link_by_language(self):
        ...
        for post in content['posts']:
            self.assertTrue(
                'polski' in post['link'] and 'link' in post['link'])

        content = json.loads(english_response.content)
        for post in content['posts']:
            self.assertTrue(
                'english' in post['link'] and 'link' in post['link'])

    def test_can_select_given_amount_of_posts_query_string(self):
        c = Client()
        response = c.get('/get_posts/?count=2',
    ...
    def test_can_select_by_category(self):
        c = Client()
        response = c.get('/get_posts/?category=cats',
                         HTTP_X_REQUESTED_WITH='XMLHttpRequest')
        content = json.loads(response.content)
        self.assertTrue(2 == len(content['posts']))
